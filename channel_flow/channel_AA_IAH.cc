...
public:
  ...
  void run(const unsigned int n_refinements, const double tolerance,
...
private:
  ...
  void run_iterations(const unsigned int n_refinements, const double tolerance,
                      const unsigned int max_iteration,
    ...
    if (cell->at_boundary()) {
      for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f) {
        if (cell->face(f)->at_boundary()) {
          ...
          if (std::abs(center[0] - 0.0) < 1e-6) // left wall (Inlet)
            cell->face(f)->set_boundary_id(1);
          else if (std::abs(center[0] - 30.0) < 1e-6) // right wall (Outlet)
            cell->face(f)->set_boundary_id(2);
          else // top, bottom and obstacle walls (No-slip)
...
template <int dim>
void ChannelFlow<dim>::Anderson_Acceleration(FullMatrix<double> &F_temp,
                                             FullMatrix<double> &u_tilde_temp,
                                             BlockVector<double> &solution,
                                             BlockVector<double> &old_solution,
                                             BlockVector<double> &AA_solution,
  ...
  if (AA_iter > 1) {
    ...
    if (AA_iter == 2) {
      ...
      int m_iter = AA_iter - 1;

      FullMatrix<double> sym_mat(m_iter, m_iter);
      Vector<double> rhs(m_iter);
      Vector<double> temp(dof_handler.n_dofs());
      Vector<double> FtM(dof_handler.n_dofs());
      for (int i = 0; i < m_iter; i++) {
        for (unsigned int k = 0; k < dof_handler.n_dofs(); k++) {
          rhs(i) -= (F(k, i) - F(k, m_iter)) * F(k, m_iter);

          for (int j = 0; j < m_iter; j++) {
            sym_mat(i, j) +=
                (F(k, i) - F(k, m_iter)) * (F(k, j) - F(k, m_iter));
          }
        }
      }

      FullMatrix<double> sym_mat_inv(m_iter, m_iter);
      sym_mat_inv.invert(sym_mat);

      Vector<double> alpha_new(m_iter);
...
template <int dim>
void ChannelFlow<dim>::run_iterations(const unsigned int n_refinements,
  ...
  while ((first_step || (current_res > tolerance)) &&
    ...
    std::cout << "**********" << std::endl;
    std::cout << " The relative error = " << current_res << " at "
              << th_iteration << "th iteration" << std::endl;

    std::ofstream res_file("residue_history.csv", std::ios_base::app);
    res_file << Re << "," << rho << "," << alpha << "," << m << ","
             << n_refinements << ",AA-IAH(m=" << m << ")," << th_iteration
             << "," << current_res << "\n";
    res_file.close();
...
void ChannelFlow<dim>::output_results(const unsigned int output_index) const {
  ...
  filename << "Channel_Re_" << Re << "_rho_" << rho << "_m_" << m
           << "_solution_" << Utilities::int_to_string(output_index, 4)
           << ".vtk";
...
template <int dim>
void ChannelFlow<dim>::run(const unsigned int n_refinements,
                           const double tolerance,
                           const unsigned int max_iteration) {
  ...
  run_iterations(n_refinements, tolerance, max_iteration, true);
}
} // end namespace MyNSE
...
int main(int argc, char **argv) {
  try {
    ...
    for (const int m : m_list) {
      std::cout << "\n======================================================\n";
      std::cout << "Running AA-IAH Channel Flow: Re = " << parameters.Re
                << ", rho = " << parameters.rho
                << ", alpha = " << parameters.alpha << ", m = " << m
      ...
      std::ofstream out_file("performance_summary.csv", std::ios_base::app);
      out_file << parameters.Re << "," << parameters.rho << ","
               << parameters.alpha << "," << m << "," << parameters.refinements
               << "," << algorithm << "," << flow.get_final_iter() << ","
               << timer.cpu_time() << "\n";
      out_file.close();
    ...
    std::cout << "\n******************************************************\n";
    std::cout << "All combinations processed. Output saved to "
                 "'performance_summary.csv' and 'residue_history.csv'"
              << std::endl;
    std::cout << "******************************************************\n\n";

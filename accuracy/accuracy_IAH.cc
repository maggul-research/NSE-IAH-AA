 ...
 *
 * This is the implementation file for the accuracy analysis
 * of the improved Arrow Hurwicz Method (IAH) for Navier Stokes Equation.
 * ---------------------------------------------------------------------
 */
...
private:
  ...
  */
  ...
  void compute_errors(BlockVector<double> solution);
...
void AccuracyTest<dim>::run_iterations(const double tolerance,
  ...
  while ((current_res > tolerance) && th_iteration < max_iteration) {
...
template <int dim>
void AccuracyTest<dim>::compute_errors(BlockVector<double> solution) {
  ...
  VectorTools::integrate_difference(
      dof_handler, solution, exact_solution_gradient, cellwise_errors,
  ...
  for (const auto &cell : dof_handler.active_cell_iterators()) {
    ...
    fe_values_div[velocities].get_function_divergences(solution, div_uh);
...
int main() {
  try {
    ...
    for (unsigned int N = 2; N <= 8; ++N) {
      std::cout << "\n======================================================\n";
      std::cout << "Running IAH for Refinement Level N = " << N
                << " (rho = " << rho << ")\n";
      std::cout << "======================================================\n";
    ...
    std::cout << "\n******************************************************\n";
    std::cout << "IAH completed for all refinement levels!" << std::endl;
    std::cout << "******************************************************\n\n";

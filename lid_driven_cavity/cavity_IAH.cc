 ...
 * IAH file printing results for NSE.
 * ---------------------------------------------------------------------
 */
...
unsigned int CavityFlow<dim>::run_iterations(const double tolerance,
  ...
  while ((first_step || current_residue > tolerance) &&
    ...
    res_file << Re << "," << rho << "," << alpha << ",0," << th_iteration << ","
             << current_residue << "\n";
    res_file.close();
...
void CavityFlow<dim>::output_results(const unsigned int output_index) const {
  ...
  filename << "Cavity_Re_" << Re << "_rho_" << rho << "_m_0_solution_"
...
template <int dim> void CavityFlow<dim>::output_centerline_profiles() const {
  ...
  basename << "centerline_Re_" << Re << "_rho_" << rho << "_m_0";
  ...
  for (unsigned int i = 0; i <= n_intervals; ++i) {
    ...
    VectorTools::point_value(dof_handler, solution, Point<dim>(0.5, coordinate),
                             values);
    vertical_file << coordinate << "," << values(0) << "\n";

    VectorTools::point_value(dof_handler, solution, Point<dim>(coordinate, 0.5),
                             values);
...
int main(int argc, char **argv) {
  try {
    ...
    std::cout << "\n======================================================\n";
    std::cout << "Running cavity IAH: Re = " << parameters.Re
              << ", rho = " << parameters.rho
    ...
    CavityFlow<2> flow(degree, parameters.Re, parameters.rho, parameters.alpha);

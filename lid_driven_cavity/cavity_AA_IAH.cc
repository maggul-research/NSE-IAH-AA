 ...
 * File printing results for NSE with Anderson Acceleration.
 * ---------------------------------------------------------------------
 */
...
void print_usage(const char *program) {
  std::cout
      ...
      << "  --alpha VALUE           pressure parameter alpha (default: 1)\n"
      << "  --m N                   one AA depth (default: run m=1,...,5)\n"
      << "  --refinements N         global refinement level (default: 8)\n"
...
RunParameters parse_arguments(const int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (argument == "--re")
      ...
      parameters.memory_depth = std::stoi(require_value());
  ...
  if (!std::isfinite(parameters.Re) || !std::isfinite(parameters.rho) ||
      ...
      parameters.tolerance <= 0.0 || parameters.memory_depth < 0)
    throw std::invalid_argument(
        "Re, rho, alpha, and tolerance must be finite and positive; "
        "m must be positive when specified.");
...
template <int dim> class CavityFlow {
public:
  CavityFlow(const unsigned int degree, const double Re, const double rho,
             const double alpha, const int m);
  unsigned int run(const unsigned int n_refinements, const double tolerance,
                   const unsigned int max_iteration);
  void output_centerline_profiles() const;
...
private:
  ...
  */
  ...
  unsigned int run_iterations(const double tolerance,
                              const unsigned int max_iteration,
...
public:
  ...
  virtual void vector_value(const Point<dim> &p,
    ...
    values(0) -= 0.25 * std::pow((1 - cos((Mpi * temp / 0.1))), 2.0);
  }
};

template <int dim>
CavityFlow<dim>::CavityFlow(const unsigned int degree, const double Re,
                            const double rho, const double alpha, const int m)
...
template <int dim>
void CavityFlow<dim>::mesh_jobs(const unsigned int n_refinements) {
    ...
    if (cell->at_boundary())
      for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
        if (cell->face(f)->at_boundary()) {
          ...
          if (fabs(p(1) - 1.0) < 1e-10)
            cell->face(f)->set_boundary_id(1);
        }
}

template <int dim> void CavityFlow<dim>::setup_dofs() {
  system_matrix.clear();
...
template <int dim> void CavityFlow<dim>::initialize_system() {
  {
...
template <int dim> void CavityFlow<dim>::assemble(int assemble_id) {
  if (assemble_id == 0) {
    system_matrix = 0;
    system_rhs = 0;
  }
...
template <int dim> void CavityFlow<dim>::solve(int solve_id) {
...
template <int dim>
void CavityFlow<dim>::Anderson_Acceleration(FullMatrix<double> &F_temp,
                                            FullMatrix<double> &u_tilde_temp,
                                            BlockVector<double> &solution,
                                            BlockVector<double> &old_solution,
                                            BlockVector<double> &AA_solution,
  ...
  if (AA_iter > 1) {
    ...
    if (AA_iter == 2) {
      ...
      FullMatrix<double> sym_mat(m_val, m_val);
      Vector<double> rhs(m_val);
      Vector<double> temp(dof_handler.n_dofs());
      Vector<double> FtM(dof_handler.n_dofs());
      for (int i = 0; i < m_val; i++) {
        for (unsigned int k = 0; k < dof_handler.n_dofs(); k++) {
          rhs(i) -= (F(k, i) - F(k, m_val)) * F(k, m_val);

          for (int j = 0; j < m_val; j++) {
            sym_mat(i, j) += (F(k, i) - F(k, m_val)) * (F(k, j) - F(k, m_val));
          }
        }
      }

      FullMatrix<double> sym_mat_inv(m_val, m_val);
      sym_mat_inv.invert(sym_mat);

      Vector<double> alpha_new(m_val);
    ...
    AA_solution = solution;
}

template <int dim>
unsigned int CavityFlow<dim>::run_iterations(const double tolerance,
                                             const unsigned int max_iteration,
                                             const bool output_result) {
  int AA_iter = 1;
  ...
  while ((first_step || (current_res > tolerance)) &&
    ...
    current_res =
        max(residue_vector.block(0).l2_norm() / AA_solution.block(0).l2_norm(),
            residue_vector.block(1).l2_norm() / AA_solution.block(1).l2_norm());

    ++th_iteration;

    std::ofstream res_file("Residual_History.csv", std::ios::app);
    res_file << Re << "," << rho << "," << alpha << "," << m << ","
             << th_iteration << "," << current_res << "\n";
    res_file.close();
...
template <int dim>
void CavityFlow<dim>::output_results(const unsigned int output_index) const {
  ...
  filename << "Cavity_Re_" << Re << "_rho_" << rho << "_m_" << m << "_solution_"
...
template <int dim>
unsigned int CavityFlow<dim>::run(const unsigned int n_refinements,
  ...
  return run_iterations(tolerance, max_iteration, true);
}

template <int dim> void CavityFlow<dim>::output_centerline_profiles() const {
  constexpr unsigned int n_intervals = 100;

  std::ostringstream basename;
  basename << "centerline_Re_" << Re << "_rho_" << rho << "_m_" << m;

  std::ofstream vertical_file(basename.str() + "_x.csv");
  std::ofstream horizontal_file(basename.str() + "_y.csv");
  vertical_file << "Coordinate,Velocity\n";
  horizontal_file << "Coordinate,Velocity\n";
  vertical_file << std::setprecision(17);
  horizontal_file << std::setprecision(17);

  Vector<double> values(dim + 1);
  for (unsigned int i = 0; i <= n_intervals; ++i) {
    const double coordinate =
        static_cast<double>(i) / static_cast<double>(n_intervals);

    VectorTools::point_value(dof_handler, AA_solution,
                             Point<dim>(0.5, coordinate), values);
    vertical_file << coordinate << "," << values(0) << "\n";

    VectorTools::point_value(dof_handler, AA_solution,
                             Point<dim>(coordinate, 0.5), values);
    horizontal_file << coordinate << "," << values(1) << "\n";
  }
}
} // end namespace MyNSE
...
int main(int argc, char **argv) {
  ...
  try {
    ...
    const std::vector<int> m_list =
        parameters.memory_depth == 0
            ? std::vector<int>{1, 2, 3, 4, 5}
            : std::vector<int>{parameters.memory_depth};

    ensure_csv_header("Performance_Summary.csv",
                      "Re,rho,alpha,m,N,Algorithm,TotalIterations,CPUTime");
    ensure_csv_header("Residual_History.csv",
                      "Re,rho,alpha,m,Iteration,Residual");

    for (const int m : m_list) {
      const std::string algorithm = "AA-IAH(m=" + std::to_string(m) + ")";
      std::cout << "\n============================================\n";
      std::cout << "Starting cavity AA-IAH: Re = " << parameters.Re
                << " | rho = " << parameters.rho << " | " << algorithm
                << " | N = " << parameters.refinements << "\n";
      std::cout << "============================================\n";

      Timer timer;
      timer.start();

      CavityFlow<2> flow(degree, parameters.Re, parameters.rho,
                         parameters.alpha, m);
      ...
      flow.output_centerline_profiles();

      std::ofstream p_file("Performance_Summary.csv", std::ios::app);
      p_file << parameters.Re << "," << parameters.rho << ","
             << parameters.alpha << "," << m << "," << parameters.refinements
             << "," << algorithm << "," << total_iters << "," << cpu_time
             << "\n";
      p_file.close();

      std::cout << "Completed: " << total_iters << " iterations, " << cpu_time
                << " seconds.\n";
    }
